...
pub fn load_buffer_from_uri<'a>(
    env: &mut JNIEnv<'a>,
    this: &JObject<'a>,
    uri: &'a str
) -> Result<LoadedGltfBuffer> {
    let data_arr = invoke_native_callback_no_args(env, this, "loadBufferFromURI", "(Ljava/lang/String;)V")?;
    ...
    Ok(LoadedGltfBuffer::new(String::from(uri), data.iter().map(|x| *x as u8).collect()))
}
...
fn init_gltf<'a>(env: &mut JNIEnv<'a>, this: &JObject<'a>) {
    ...
    match gltf_data {
        Ok(gltf_data) => {
            ...
            match gltf_obj {
                Ok(gltf_obj) => {
                    util::jni::set_rust_field(env, this, "rust_gltfObj", gltf_obj.clone());
                }
                Err(err) => {
                    util::jni::clear_exception_if_occurred(env);
